    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Article Images (Max {maxImages})</label>
                <span className="text-xs text-gray-500 dark:text-gray-400">{images.length}/{maxImages} Uploaded</span>
            </div>

            {/* Dropzone area */}
            {images.length < maxImages && (
                <div className="border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-lg p-6 text-center hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors bg-white dark:bg-slate-900/50">
                    <input
                        type="file"
                        id="image-upload"
                        className="hidden"
                    ...
                    <label htmlFor="image-upload" className="cursor-pointer flex flex-col items-center">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400">Click to upload JPG, PNG, WEBP</span>
                    </label>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-4">
                {images.map((img) => {
                    ...
                    return (
                        <div key={uniqueId} className="border border-gray-200 dark:border-slate-700 rounded-lg p-4 bg-white dark:bg-slate-900 relative shadow-sm transition-colors">
                            <button
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <div className="flex-shrink-0">
                                    <img
                                        ...
                                        className="w-32 h-32 object-cover rounded-md border border-gray-300 dark:border-slate-700 bg-gray-50 dark:bg-slate-950 text-gray-500 dark:text-gray-400 text-xs flex items-center justify-center p-2"
                                    />
                                    <div className="mt-2 text-center">
                                        <button
                                            onClick={() => toggleFeatured(uniqueId)}
                                            className={`text-xs px-2 py-1 rounded-full border transition-colors ${img.isFeatured
                                                ? 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-900/50'
                                                : 'bg-white dark:bg-slate-950 text-gray-600 dark:text-gray-400 border-gray-300 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'
                                                }`}
                                        >
                                ...
                                <div className="flex-grow space-y-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1">Caption (Marathi)</label>
                                        <input
                                            type="text"
                                            value={img.caption || ''}
                                            onChange={(e) => updateMeta(uniqueId, 'caption', e.target.value)}
                                            className="w-full text-sm p-2 border border-gray-300 dark:border-slate-700 rounded focus:ring-1 focus:ring-blue-500 bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-gray-100 transition-colors placeholder:text-gray-400 dark:placeholder:text-gray-600"
                                            placeholder="Enter caption..."
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1">Alt Text (Marathi)</label>
                                        <input
                                            type="text"
                                            value={img.altText || ''}
